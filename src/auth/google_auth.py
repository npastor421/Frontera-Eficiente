...
def render_user_auth_topbar() -> None:
    """
    Render a compact, stylish Google Sign-In button or profile popover in the top-right corner.
    """
    user = get_active_user()
    oauth_ready = is_oauth_configured()
    ...
    if user:
        ...
        with st.popover(f"👤 {first_name}", use_container_width=True):
            if picture:
                st.markdown(
                    f"<div style='text-align: center; margin-bottom: 8px;'><img src='{picture}' style='width: 48px; height: 48px; border-radius: 50%; border: 2px solid #00FF66;' /></div>",
                    unsafe_allow_html=True,
                )
            st.markdown(f"**{name}**")
            st.caption(email)
            st.markdown("<span style='font-size: 11px; color: #00FF66;'>● Conectado (Portafolios Privados)</span>", unsafe_allow_html=True)
            st.markdown("---")
            if st.button("🚪 Cerrar Sesión", use_container_width=True, key="topbar_logout_btn"):
                logout_user()
    else:
        if oauth_ready:
            auth_url = get_google_auth_url()
            st.markdown(
                ...
                <div style="display: flex; justify-content: flex-end; align-items: center; height: 100%;">
                    <a href="{auth_url}" target="_self" title="Iniciar sesión con Google" style="text-decoration: none;">
                        <div style="background: #FFFFFF; color: #3c4043; border-radius: 20px; padding: 6px 14px; display: inline-flex; align-items: center; gap: 8px; font-weight: 600; font-size: 13px; box-shadow: 0 2px 5px rgba(0,0,0,0.25); border: 1px solid #dadce0; cursor: pointer; transition: all 0.2s ease;">
            ...
            with st.popover("👤 Invitado", use_container_width=True):
                st.caption("Modo local: Portafolios guardados en este navegador.")
                demo_email = st.text_input("Correo Demo", value="amigo@gmail.com", key="top_demo_email")
                demo_name = st.text_input("Nombre Demo", value="Inversionista", key="top_demo_name")
                if st.button("Simular Login", use_container_width=True, key="top_btn_demo_login"):
                    st.session_state["user"] = {
                        "id": "demo_123",
                        "email": demo_email.strip(),
                        "name": demo_name.strip(),
